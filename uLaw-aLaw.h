...
void init_ulaw_code_value()
{
  for (int i=0; i<256; i++)
  {
    ...
    ulaw_code_value[i] = sign ? 0x84 - (noise << scale)
                              : (noise << scale) - 0x84;
  }
}
...
unsigned char ulaw_encode_sample(short sample)
{
...
unsigned char alaw_encode_sample(short sample)
{
  unsigned char uhigh = ((unsigned short)sample) >> 8;
...
short ulaw_decode_sample(unsigned char sample)
{
  return ulaw_code_value[sample];
}

short alaw_decode_sample(unsigned char sample)
{
